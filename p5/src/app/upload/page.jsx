...
export default function UploadPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const videoData = await uploadVideoToStorage(user.id, videoFile);

      // Step 2: Upload thumbnail to Supabase (if provided)
      let thumbnailData = null;
      if (thumbnailFile) {
        toast.loading('Uploading thumbnail...');
        thumbnailData = await uploadThumbnailToStorage(user.id, thumbnailFile);
      }

      // Step 3: Send metadata (URLs) to your backend
      toast.loading('Saving video...');
      await createVideo({
        caption,
        videoUrl: videoData.url,
        videoStoragePath: videoData.storagePath,
        thumbnailUrl: thumbnailData?.url || null,
        thumbnailStoragePath: thumbnailData?.storagePath || null,
      });
